...
const RelationModal: React.FC<{ node: any; onClose: () => void }> = ({ node, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${domain}:8000/api/v1/relations/detail/${node.target.__data__.friend_uid}/`
        );
    ...
    axios
      .get(`${domain}:8000/api/v1/users/info/${node.target.__data__.friend_uid}/`)
      .then((response) => {
        // 로그인 성공 시 처리
